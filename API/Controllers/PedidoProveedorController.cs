...
namespace API.Controllers
{
    ...
    public class PedidoProveedorController : ControllerBase
    {
        ...
        [HttpGet("pdf-dia-fecha")]
        public async Task<IActionResult> GenerarPdfPorFecha([FromQuery] DateTime fecha, [FromQuery] int idProveedor)
        {
            try
            {
                var pdfBytes = await _IPedidoProveedorBussines.GenerarPdfPedidosPorFecha(fecha, idProveedor);
                return File(pdfBytes, "application/pdf", $"Pedidos_{fecha:yyyyMMdd}.pdf");
            }
            catch (Exception ex)
            {
        ...
        [HttpGet("pdf-link-firebase")]
        public async Task<IActionResult> ObtenerPdfTemporalFirebase([FromQuery] DateTime fecha, [FromQuery] int idProveedor)
        {
            try
            {
                var url = await _IPedidoProveedorBussines.GenerarYSubirPdfTemporal(fecha, idProveedor);
                return Ok(new { url });
            }
            catch (Exception ex)
            {
