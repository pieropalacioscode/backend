...
namespace Bussines
{
    public class LibroBussines : ILibroBussines
    {
        ...
        public async Task<PaginacionResponse<LibroResponse>> ListarPorProveedorPaginadoAsync(int idProveedor, int pagina, int cantidad)
        {
            var resultado = await _ILibroRepository.ListarPorProveedorPaginadoAsync(idProveedor, pagina, cantidad);
            var dtoList = _Mapper.Map<List<LibroResponse>>(resultado.Items);

            return new PaginacionResponse<LibroResponse>
            {
                Items = dtoList,
